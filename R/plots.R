...
ggplot(dat, aes(frequency, rt, colour = participant)) +
  ...
  stat_summary(aes(y = predict(model_int), group = 1), fun.data = "mean_se", geom = "ribbon", colour = NA, alpha = 0.5) +
  ...
  annotate(
    x = 1.5, y = 3, colour = "black", geom = "label",
    label = paste0("Intercept: ", round(fixef(model_slo)[1], 2), "\nCoefficient: ", round(fixef(model_slo)[1], 2))) +
  geom_text(
...
animate(
  predictions_animate, height = 2000, width = 2000, 
